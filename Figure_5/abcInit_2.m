...
function [ ControlData, Data] = abcInit( Data )
       ...
       ControlData.IsCompensate = 1; % 是否补偿[ 6;6;40;0.965;0.965; 9.65]*1;
       ControlData.Bound = [ 12; 12; 40; 9.6; 9.6; 9.6]*1; %[ 3;3;32;0.65;0.65;0.65]
      ...
      %% ---------adrc Params Begin  
          ...
          %% --------- ESO
           ...
           w0 = 3.2;%(2018.04.29 :1.2)
           Params.Beta  = ones(n+1,m).* [3*w0; 3*w0^2; w0^3]; %[100; 300; 1000];% .*[1/h; 1/(3*h^2); 1/(64* h^3) ]; % ones(n+1,m)%另有 100 300 1000
           Params.Delta = ones(1,m).*h*1;
           Params.b0     = [1,1,1];
           Params.esoType ='n' ; % 非线性
           
          %% --------- NFL
           Params.cK = [ 12.0,   12.0,    1.8;...
                         18.0,   18.0,    9.6]; %  
%            Params.cK = ones(n,m).*[19.1; 6.1];
           Params.cAlpha = ones(n,m).*[0.9;3.6]; % 0<α1<1<αi, i=2,3,4....
           Params.cDelta = ones(1,m).*[0.2,0.2,0.2]; % 04.19:[0.2,0.2,0.2];
           
           Params.cr  = [1,1,1];
           Params.cC  = [ 2.2,1.2,1.2]; % 相当于 d
           Params.ch1 = [ 0.12, 0.12, 0.001] ; % 1/h1 相当于p (2018.04.19:[ 0.6, 0.6, 0.3])
...
%% Backstepping Attitude
           ...
           ControlData.bsParamsR.Alpha2 = [ 9; 9; 19 ];
           ControlData.bsParamsR.ki_1   = [ 1.0;1.0;1.0]*0.1;
           ControlData.bsParamsR.ki_2   = [ 1.0;1.0;1.0]*0.1;
